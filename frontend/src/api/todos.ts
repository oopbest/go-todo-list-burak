...
export function getTodoByID(id: string, signal?: AbortSignal): Promise<Todo> {
  return request<Todo>(`${TODOS_API}/${encodeURIComponent(id)}`, { signal });
}
...
export function updateTodo(id: string, input: UpdateTodoInput): Promise<Todo> {
  return request<Todo>(`${TODOS_API}/${encodeURIComponent(id)}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
    },
...
export function deleteTodo(id: string): Promise<void> {
  return request<void>(`${TODOS_API}/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
